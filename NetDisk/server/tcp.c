...
extern pthread_mutex_t epfd_mutex;
...
void EpollAdd(int epfd, int fd, uint32_t EVENTS){
	...
	evt.data.fd = fd;
	evt.events = EVENTS;
    int ret = pthread_mutex_lock(&epfd_mutex);
    THREAD_ERROR_CHECK(ret, "pthread_epfd_mutex_lock");
    
    if(epoll_ctl(epfd, EPOLL_CTL_ADD, fd, &evt) < 0) {
		perror("epoll_ctl");
	}
    ret = pthread_mutex_unlock(&epfd_mutex);
    THREAD_ERROR_CHECK(ret, "pthread_epfd_mutex_unlock");
}
...
//通过小火车的形式给peerfd发送sendbuf中的内容
int  send_sendbuf(int peerfd,char*sendbuf){

    train_t train;
    memset(&train,0,sizeof(train_t));
    train.length=strlen(sendbuf);
    strncpy(train.data,sendbuf,strlen(sendbuf));
    printf("train.data=%s\n",train.data);
    int ret=send(peerfd,&train,4+train.length,0);
    ERROR_CHECK(ret,-1,"send cd result");
    printf("send:train ret=%d\n",ret);
    return ret;
}
